...
func findWordPosition(dictionary []wordCount, word string) (int, int64) {
    var totalTime int64 
    timeStart := time.Now()
...
func removeFromSlice(arrayWordCount []wordCount, itToRemove int) ([]wordCount, int64) {
	// Remove the element at index itToRemove from the slice
    removingStart := time.Now()
	previous := arrayWordCount[:itToRemove]
	following := arrayWordCount[itToRemove+1:]
	final := append(previous, following...)
    removingTime := time.Since(removingStart)
	return final, removingTime.Nanoseconds()
}
...
func mergeCounts(arr_1, arr_2 []wordCount) []wordCount {
	...
	for i, wC_1 := range arr_1 {
		...
		if found > -1 {
            var timeToRemove int64
			arr_2, timeToRemove = removeFromSlice(arr_2, found)
            totalRemovalTime += timeToRemove
		}
	}
...
func countWords(words []string) []wordCount {
    ...
    for _, word := range words {
        ...
        if len(wordCountDict) == 0 {
            ...
            wordWithCountPosition, searchTime := findWordPosition(wordCountDict, word)
